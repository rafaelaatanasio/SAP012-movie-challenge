import App from './App.js'; // Importa o padrão exportado de App.js
import { getMovies, getId } from '../lib/tmdb.js';  // Importe as funções que deseja testar
import fetchMock from 'jest-fetch-mock';

// Configura o fetch mock para ser usado em todos os testes
...
const mockData = {
    ...
    "total_pages": 43871,
    "total_results": 877411
};

describe('App', () => {
    it('deve renderizar sem falhas', () => {
        // Renderizar o componente App com os dados fictícios
        const el = App(mockData);
        // Verificar se o elemento retornado é uma instância de HTMLElement
...
describe('getMovies', () => {
    it('deve retornar uma lista de filmes', async () => {
        ...
        // Verificar se uma lista de filmes foi retornada
        expect(movies).toBeDefined();
        expect(Array.isArray(movies.results)).toBe(true);
        // Adicione mais asserções conforme necessário para verificar outros detalhes dos filmes retornados
    });

});

describe('getId', () => {
    it('deve retornar os detalhes de um filme com base no ID fornecido', async () => {
        fetch.mockResponseOnce(JSON.stringify(mockData.results[0]))
        // Configurar dados de entrada para o teste
        const movieId = 1096197;

        // Chamar a função a ser testada
        const movieDetails = await getId(movieId);

        // Adicione um console.log() para visualizar os detalhes do filme retornado
        console.log('Detalhes do filme:', movieDetails);

        // Verificar se os detalhes do filme foram retornados corretamente
        expect(movieDetails).toBeDefined();
        expect(movieDetails.id).toBe(movieId);
        // Adicione mais asserções conforme necessário para verificar outros detalhes do filme
    });
});
